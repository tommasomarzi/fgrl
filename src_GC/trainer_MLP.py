...
from models.baselines.MLP_model import MLP_BL
...
def simulate(sol_MLP, ep_rewards, ep_timesteps, model_MLP, seed, cfg, envs):
    # set random seeds
    ...
    model_MLP = utils.update_params_total(model_MLP, sol_MLP)

    while (not done) and (t < cfg["environment"]["max_episode_steps"]):
        t += 1

        with torch.no_grad():
            current_step = obs[0][cfg["environment"]["n_connected_comp"]]                      # get current time id
            obs = np.delete(obs, cfg["environment"]["n_connected_comp"], 1).flatten()          # remove time id
            obs = np.append(obs, current_step)                                                  # add time id to the end
            obs = torch.from_numpy(obs).float()
            actions = model_MLP(obs)
            actions = actions.reshape(cfg["environment"]["nodes_per_cc"]*cfg["environment"]["n_connected_comp"], -1)
...
def evaluate_batch(sol_MLP, model_MLP, cfg, envs, seeder=None):
    ...
    if cfg["utilities"]["multiprocessing"][0]:
        input_args = [(sol_MLP, ep_rewards[pop_id], ep_timesteps[pop_id], copy.deepcopy(model_MLP), seeds[pop_id],
        ...
        for pop_id in range(cfg["optimizer"]["pop_size"]):
            pool_results.append(simulate(sol_MLP, ep_rewards[pop_id], ep_timesteps[pop_id],
                                         copy.deepcopy(model_MLP), seeds[pop_id], cfg, envs))
...
class Trainer_MLP:
    def __init__(self, config, save_exp=True):
        ...
        # remark: add_softmax has no effects
        input_dim = (self.node_feats_dim - 1)*self.n_nodes + 1          # encode time id only once
        mu = create_model(input_size=input_dim,
                          output_size=self.action_dim*self.n_nodes, layers=hyperparams["internal_dim"],
                          last_activation="none")

        self.model_MLP = MLP_BL(mu=mu, seed=self.seed)
        ...
        num_params = self.model_MLP.get_model_num_params()
        config["num_params_model"] = [num_params]
        self.es_MLP = CMAES(num_params, init_params, sigma_init=init_sigma, popsize=self.pop_size, seed=self.seed)
    ...
    def train(self):
        ...
        for episode in range(self.cfg["simulation"]["num_episodes"]):
            ...
            sols_MLP = self.es_MLP.ask()      # get parameters from es
            ...
            if self.cfg["utilities"]["multiprocessing"][0]:
                input_args = [(sols_MLP[pop_id], ep_rewards[pop_id],
                               ep_timesteps[pop_id], copy.deepcopy(self.model_MLP), seeds[pop_id], self.cfg,
                               self.envs)
                ...
                pool_results = []
                for pop_id in range(self.pop_size):
                    input_args = (sols_MLP[pop_id],
                                  ep_rewards[pop_id], ep_timesteps[pop_id], copy.deepcopy(self.model_MLP),
            ...
            self.model_MLP, self.es_MLP = utils.run_es_update_params_total(self.model_MLP, self.es_MLP, ep_rewards)
            self.es_MLP = self.es_MLP[0]      # run_es_update_params_total() returns a list

            # Evaluate current params and update best reward
            if episode % self.cfg["simulation"]["eval_episodes_freq"] == 0:
                sols_MLP = np.array(self.es_MLP.current_param()).round(4)
                reward_eval, ep_timestep_eval, solved_eval = evaluate_batch(sols_MLP, copy.deepcopy(self.model_MLP),
                                                                            self.cfg, self.envs,
                                                                            seeder=self.seeder_eval)
                if self.cfg["simulation"]["exp_id"] != 0:
                    save_dict = {}
                    save_dict['params_sols'] = np.array(self.es_MLP.current_param()).round(4)
                    save_dict['sigma'] = np.array(self.es_MLP.current_sigma()).round(4)
                    save_dict['rms_sigma'] = np.array(self.es_MLP.rms_stdev()).round(4)
