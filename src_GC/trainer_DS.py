...
def simulate(sol_DS, ep_rewards, ep_timesteps, model_DS, seed, cfg, envs):
    # set random seeds
    ...
    model_DS = utils.update_params_total(model_DS, sol_DS)
...
def evaluate_batch(sol_DS, model_DS, cfg, envs, seeder=None):
    ...
    if cfg["utilities"]["multiprocessing"][0]:
        input_args = [(sol_DS, ep_rewards[pop_id], ep_timesteps[pop_id], copy.deepcopy(model_DS), seeds[pop_id],
        ...
        for pop_id in range(cfg["optimizer"]["pop_size"]):
            pool_results.append(simulate(sol_DS, ep_rewards[pop_id], ep_timesteps[pop_id],
                                         copy.deepcopy(model_DS), seeds[pop_id], cfg, envs))
...
class Trainer_DS:
    def __init__(self, config, save_exp=True):
        ...
        mu = create_model(input_size=self.node_feats_dim,
                          output_size=self.action_dim, layers=hyperparams["internal_dim"],
                          last_activation="softmax")

        self.model_DS = DeepSet_BL(mu=mu, seed=self.seed)
        ...
        num_params = self.model_DS.get_model_num_params()
        config["num_params_model"] = [num_params]
        self.es_DS = CMAES(num_params, init_params, sigma_init=init_sigma, popsize=self.pop_size, seed=self.seed)
    ...
    def train(self):
        ...
        for episode in range(self.cfg["simulation"]["num_episodes"]):
            ...
            sols_DS = self.es_DS.ask()      # get parameters from es
            ...
            if self.cfg["utilities"]["multiprocessing"][0]:
                input_args = [(sols_DS[pop_id], ep_rewards[pop_id],
                               ep_timesteps[pop_id], copy.deepcopy(self.model_DS), seeds[pop_id], self.cfg,
                               self.envs)
                ...
                pool_results = []
                for pop_id in range(self.pop_size):
                    input_args = (sols_DS[pop_id],
                                  ep_rewards[pop_id], ep_timesteps[pop_id], copy.deepcopy(self.model_DS),
            ...
            self.model_DS, self.es_DS = utils.run_es_update_params_total(self.model_DS, self.es_DS, ep_rewards)
            self.es_DS = self.es_DS[0]      # run_es_update_params_total() returns a list

            # Evaluate current params and update best reward
            if episode % self.cfg["simulation"]["eval_episodes_freq"] == 0:
                sols_DS = np.array(self.es_DS.current_param()).round(4)
                reward_eval, ep_timestep_eval, solved_eval = evaluate_batch(sols_DS, copy.deepcopy(self.model_DS),
                                                                            self.cfg, self.envs,
                                                                            seeder=self.seeder_eval)
                if self.cfg["simulation"]["exp_id"] != 0:
                    save_dict = {}
                    save_dict['params_sols'] = np.array(self.es_DS.current_param()).round(4)
                    save_dict['sigma'] = np.array(self.es_DS.current_sigma()).round(4)
                    save_dict['rms_sigma'] = np.array(self.es_DS.rms_stdev()).round(4)
