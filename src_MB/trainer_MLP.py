...
def simulate(sol_MLP, ep_rewards, ep_timesteps, model_MLP, seed, cfg, envs):
    ...
    model_MLP = utils.update_params_total(model_MLP, sol_MLP)    # Update model parameters using es solution
...
def evaluate_batch(sol_MLP, model_MLP, cfg, envs, seeder=None):
    ...
    if cfg.multiprocessing[0]:
        input_args = [(sol_MLP, ep_rewards[pop_id], ep_timesteps[pop_id], copy.deepcopy(model_MLP),
...
class Trainer_MLP:
    def __init__(self, config, save_exp=False):
        ...
        if self.cfg.single_optimizer or self.cfg.no_intrinsic_reward:
            raise ValueError("Ablations not supported for MLP baseline")
        ...
        mu = create_model(input_size=self.cfg.limb_obs_size*self.cfg.max_num_limbs,
                          output_size=self.cfg.max_num_limbs, layers=self.cfg.mu_layers)

        self.model_MLP = MLP_BL(mu=mu, seed=self.cfg.seed)
        ...
        self.es_MLP = CMAES(num_params, init_params, sigma_init=self.cfg.sigma_init, popsize=self.pop_size,
                             seed=self.cfg.seed)
    ...
    def train(self):
        ...
        for episode in range(self.cfg.num_episodes):
            ...
            if self.cfg.multiprocessing[0]:
                input_args = [(sols_MLP[pop_id], ep_rewards[pop_id], ep_timesteps[pop_id],
                               copy.deepcopy(self.model_MLP), seeds[pop_id], self.cfg, self.envs)
            ...
            self.es_MLP = utils.run_es_update_params_total(self.es_MLP, ep_rewards)
            ...
            if episode % self.cfg.eval_episodes == 0:
                ...
                reward_eval, ep_timestep_eval = evaluate_batch(sols_MLP, copy.deepcopy(self.model_MLP),
