...
from models.baselines.DS_model import DeepSet_BL
...
def simulate(sol_deepset, ep_rewards, ep_timesteps, model_deepset, seed, cfg, envs):
    # set random seeds
    ...
    model_deepset = utils.update_params_total(model_deepset, sol_deepset)    # Update model parameters using es solution
...
def evaluate_batch(sol_deepset, model_deepset, cfg, envs, seeder=None):
    ...
    if cfg.multiprocessing[0]:
        input_args = [(sol_deepset, ep_rewards[pop_id], ep_timesteps[pop_id], copy.deepcopy(model_deepset),
        ...
        for pop_id in range(cfg.pop_size):
            pool_results.append(simulate(sol_deepset, ep_rewards[pop_id], ep_timesteps[pop_id],
                                         copy.deepcopy(model_deepset), seeds[pop_id], cfg, envs))
...
class Trainer_DS:
    def __init__(self, config, save_exp=False):
        ...
        if self.cfg.single_optimizer or self.cfg.no_intrinsic_reward:
            raise ValueError("Ablations not supported for DeepSet baseline")
        ...
        mu = create_model(input_size=self.cfg.limb_obs_size,
                          output_size=self.cfg.mu_out, layers=self.cfg.mu_layers)

        self.model_deepset= DeepSet_BL(mu=mu, seed=self.cfg.seed)
        ...
        num_params = self.model_deepset.get_model_num_params()
        config.num_params_model.append(num_params)
        self.es_deepset = CMAES(num_params, init_params, sigma_init=self.cfg.sigma_init, popsize=self.pop_size,
                                seed=self.cfg.seed)
    ...
    def train(self):
        ...
        for episode in range(self.cfg.num_episodes):
            ...
            sols_deepset = self.es_deepset.ask()     # get parameters from es
            ...
            if self.cfg.multiprocessing[0]:
                input_args = [(sols_deepset[pop_id], ep_rewards[pop_id], ep_timesteps[pop_id],
                               copy.deepcopy(self.model_deepset), seeds[pop_id], self.cfg, self.envs)
            ...
            self.es_deepset = utils.run_es_update_params_total(self.es_deepset, ep_rewards)
            self.es_deepset = self.es_deepset[0]      # run_es_update_params_total() returns a list

            # Evaluate current params and update best reward
            if episode % self.cfg.eval_episodes == 0:
                sols_deepset = np.array(self.es_deepset.current_param()).round(4)
                reward_eval, ep_timestep_eval = evaluate_batch(sols_deepset, copy.deepcopy(self.model_deepset),
                                                               self.cfg, self.envs, seeder=self.seeder_eval)
                if episode == 0:
                    ...
                    if best_reward_eval < np.mean(reward_eval):
                        best_reward_eval = np.mean(reward_eval)
                        save_dict = {}
                        save_dict['params_sols'] = np.array(self.es_deepset.current_param()).round(4)
